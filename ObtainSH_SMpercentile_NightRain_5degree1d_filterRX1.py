...
for iyear in range(year1, year2):
    ...
    filename2    = path5 + 'Obtain_NightPP_maxind_' + str(iyear) + '_24km120km_iterate_filterRX1.nc'
...
for jdim in range(lat_blocks.shape[0]):
    for idim in range(lat_blocks.shape[1]):
        ...
        for iyear in range(year1, year2):
            filename1     = path5 + 'Night_PP_max_latind_'+ str(iyear) + months[imonth] + '_24km120km_iterate_filterRX1.txt'
            filename2     = path5 + 'Night_PP_max_lonind_'+ str(iyear) + months[imonth] + '_24km120km_iterate_filterRX1.txt'
            filename3     = path5 + 'Night_PP_min_latind_'+ str(iyear) + months[imonth] + '_24km120km_iterate_filterRX1.txt'
            filename4     = path5 + 'Night_PP_min_lonind_'+ str(iyear) + months[imonth] + '_24km120km_iterate_filterRX1.txt'
            filename5     = path5 + 'Night_PP_datetime_'  + str(iyear) + months[imonth] + '_24km120km_iterate_filterRX1.txt'
            ...
            for ii in range(len(datetime_list)):      
                ...
                if (pixel_5degree[imax_lat, imax_lon] > 0.5):
                    ...
                    if (np.isnan(imax_id)):        # grid not associated with max rain
                        print('ERROR! grid not associated with max rain, iyear, ind_day, imax_lat, imax_lon, imax_id:',iyear, ind_day, imax_lat, imax_lon, imax_id)
                    else:
        ...
        if np.count_nonzero(~np.isnan(EVENT_SM1m_delta)) > 25:
            ...
            for ievent in range(event_ind+1):
                if (EVENT_MCSmask[ievent] < 1.5):  # not an MCS event
                    EVENT_SM1m_delta[ievent]      = np.nan
                    EVENT_SM1m_max[ievent]        = np.nan
                    NONEVENT_SM1m_delta[ievent,:] = np.nan
                    NONEVENT_SM1m_max[ievent,:]   = np.nan
...
#-----save output-----
outfile0   =  path5 + 'Obtain_SMpercentiles_NightRain_'+monthstr[imonth]+'_5degree1d_filterRX1.nc'
