...
for imonth in range(1,5):
    ...
    for iyear in range(year1, year2):
        filename1    = path5 + 'Obtain_PP_allpentads_'   +str(iyear)+'_160GridSmooth.nc'
        filename2    = path5 + 'Obtain_SM123_allpentads_'+str(iyear)+'_160GridSmooth.nc'
        data1        = Dataset(filename1, 'r', format='NETCDF4')
        pp0          = data1.variables['PP_total_sm']
        pptr0        = data1.variables['PP_mcs_sm']  
        ppntr0       = data1.variables['PP_nonmcs_sm']
            ...
            if (land11[iy,ix] > 0.5):
                ...
                for iyear in range(year1,year2):
                    for ipentad in range(0,5):
                        ...
                        A1[istep] = PP_TOTAL_JULY[ind1+1, iy, ix]      - PP_TOTAL_SLOPE[ipentad+1]*(iyear-year1)
                        B1[istep] = PP_MCS_JULY[ind1+1, iy, ix]        - PP_MCS_SLOPE[ipentad+1]*(iyear-year1)
                        C1[istep] = PP_NONMCS_JULY[ind1+1, iy, ix]     - PP_NONMCS_SLOPE[ipentad+1]*(iyear-year1)
    ...
    outfile0   =  path1 + '/LDASOUT_LandAtmo/Obtain_'+ monthstr[imonth] +'SM5_PP5_correlations_160GridSmooth_detrend.nc'
